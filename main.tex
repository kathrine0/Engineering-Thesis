...
\mainmatter % powoduje, ze wlasciwa czesc pracy ma strony numerowe od 1 cyframi arabskimi
	...
	\ksremark{Wstęp już wygląda dużo lepiej.}
			...
			Ludzi od lat fascynowały dalekie podróże i nieznane lądy. Już starożytni zastanawiali się co znajduje się za wielką wodą. Chyba każdy ze szkoły zna opowieść o mitycznym Odyseuszu i jego wyprawie \ksremark{Ksenofont opisał rzeczywistą wędrówkę w Azji Mniejszej, od Jonii przez Mezopotamię do wybrzeża Morza Czarnego :-)}. Kiedy w czasach renesansu pojawiła się ku temu możliwość, ludzie wsiedli na statki i wypłynęli w szerokie morze w poszukiwaniu nowych, nieznanych terenów. I tak też w 1492 roku Krzysztof Kolumb dopłynął aż do Ameryki\ksremark{przecinek} a w 1519 Ferdynand Magellan wyruszył opłynąć cały świat. 
			
			Warto jednak wspomnieć, że jeszcze do niedawna podróże zarezerwowane były dla nielicznych. Ograniczeniem były  finanse i brak szybkich środków lokomocji\ksremark{przecinek} ale i przede wszystkim \ksremark{<<przede wszystkim>> nie wiem, czy to trochę nie za mocne stwierdzenie} ludzki strach przed nieznanym. Dopiero w ciągu ostatnich kilkudziesięciu lat \ksremark{<<kilkadziesiąt lat>> zbyt wąski przedział}, wraz z rozwojem lotnictwa \ksremark{Lotnictwo był wyłączył, bo i bez samolotów ludzie podróżowali, polepszenie standardów życia było, myślę, podstawowym czynnikiem.} i ogólnym polepszeniem standardu życia, zagraniczne wakacje stały się osiągalne dla każdego. Spowodowało to gwałtowną ekspansję branży turystycznej, na którą składają się nie tylko biura podróży czy hotele\ksremark{przecinek} ale i wydawnictwa podróżnicze wydające przewodniki turystyczne \ksremark{były bardzo popularne już przed wojną}. 
	        ...
	        \ksremark{Tu jeszcze trzeba opisać organizację pracy. Rozdział \ref{id:cha:wstep} opisuje \ldots\ Rozdział \ref{id:cha:technologia} przedstawia użyte technologie, zarówno w części mobilnej (podrozdział \ref{...}), jak i webowej  (podrozdział \ref{...})  \ldots}
...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
	...
	\label{id:cha:specyfikacja_wewnetrzna}
		...
		\section{Najważniejsze algorytmy}
			...
			\ksremark{To trzeba przerobić, bo nie do końca jest poprawne, $\mathrm{haversin}(\theta) = \mathrm{sem}(\theta) = \sin^2\left(\frac{\theta}{2}\right) $ jest dość rzadko dziś używaną funkcją. Kiedyś była popularniejsza, bo obliczenia nawigacyjne i astronomiczne wykonywano ręcznie, więc tablicowano funkcje $\mathrm{haversin}(\theta)$ dla łatwiejszych rachunków.}
	...
	Testowanie aplikacji odbywało się w kilku etapach. Na etapie projektowania zostały sprecyzowane wymagania funkcjonalne, jakie każda z aplikacji powinna spełniać. Następnie na podstawie tych wymagań sformułowane zostały scenariusze testowe. W czasie rozwijania kodu, moduły testowane były manualnie. Pod koniec każdego etapu, przeprowadzane były testy akceptacyjne na podstawie scenariuszy testowych. 
	 ...
	 \ksremark{Tabele nie muszą koniecznie być w ściśle określonym miejscu. Inaczej trudno jest ładnie je poukładać, robią się duże przerwy. Z drugiej strony nie łamie się między stronami tak małych tabel. Dlatego najkorzystniej będzie tabele umieścić w obiektach pływających i umieścić w tekście odnośniki do tabel. Przykład umieściłem poniżej na niebiesko.}
	...
	\item testowanie aplikacji mobilnej:
	   \begin{itemize}
	   \item odczytanie informacji o obiekcie turystycznym z przewodnika (tab. \ref{tab:})
	   \item nawigacja z kompasem  (tab. \ref{tab:})
	   \item pobranie danych  (tab. \ref{tab:})
	   \item nieudana próba pobrania danych  (tab. \ref{tab:})
	   \end{itemize}
	\end{itemize}
	}
